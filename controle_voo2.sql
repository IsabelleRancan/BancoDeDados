...
AS $$
BEGIN
    return chr(cast(65 + random()*25 as int)); -- isso vai sortear os caracteres aleatórios
END;
$$ LANGUAGE plpgsql;
...
select * from Fn_Cria_Aeronaves_loop(33,  'A330');
...
/*
COPY (select number, gender, givenname, surname, kilograms, centimeters 
      FROM fakenames order by number limit 1000) 
TO '/dados/controle_voo/1000pessoas.csv' DELIMITER ';' CSV HEADER;


create or replace view v1000 AS select number, gender, givenname, surname, kilograms, centimeters 
      FROM fakenames order by random() limit 1000
;


\COPY (select number, gender, givenname, surname, kilograms, centimeters 
      FROM V1000 order by number) 
TO '/dados/ALGORITMOSii/funcoes/fn/1000pessoas.csv' 
WITH CSV HEADER;


\COPY (SELECT * FROM (select number, UPPER(LEFT(gender, 1)), givenname, surname, kilograms, centimeters 
      FROM fakenames order by random() limit 1000) X order by number) 
TO '/dados/ALGORITMOSii/funcoes/fn/1000pessoas.csv' 
WITH CSV DELIMITER ';';
